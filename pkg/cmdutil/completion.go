...
// CompletionOrganizations returns a completion function that provides organization
// names for shell completion.
func CompletionOrganizations(tfeClient func() (*tfc.Client, error)) func(
	*cobra.Command, []string, string,
) ([]string, cobra.ShellCompDirective) {
	return func(cmd *cobra.Command, _ []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		...
		var completions []string
		for _, org := range orgs {
			completions = append(completions, org.Name)
		}
		return completions, cobra.ShellCompDirectiveNoFileComp
	}
}

// CompletionWorkspacesFromOrgFlag returns a completion function that provides workspace
// names by reading the --org flag value to determine the organization.
func CompletionWorkspacesFromOrgFlag(tfeClient func() (*tfc.Client, error)) func(
	*cobra.Command, []string, string,
) ([]string, cobra.ShellCompDirective) {
	return func(cmd *cobra.Command, _ []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		org, _ := cmd.Flags().GetString("org")
		...
		workspaces, _, err := client.Workspaces.List(ctx, org, &tfc.WorkspaceListOptions{
			Search:      toComplete,
			ListOptions: tfc.ListOptions{Limit: 100},
		})
		if err != nil {
